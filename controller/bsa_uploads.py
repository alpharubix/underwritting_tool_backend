...
async def pdf_upload_consumer_v2(request,files,mongodb_connection,background_task):
    try:
        ...
        user_id = request.state.user_id
        requester_role = request.state.role
        cust_id = data_params.get("cust_id")
        ...
        if scoreme_response:
            ...
            # create the bsa_ref document post successfull response from the scoreme server
            await create_bsa_ref_document(user_id=user_id, reference_id=reference_id, input_data=data_params,
        ...
        return JSONResponse(status_code=status.HTTP_202_ACCEPTED, content={
            "message": "File is under processing we will let you know in the mail once the report got generated"})


    except json.decoder.JSONDecodeError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail={"message":"Invalid Request Body"})
    except HTTPException as e:
        raise e  # Re-raise HTTP exceptions as-is so FastAPI handles them correctly
    except Exception as e:
        print("Error has been raised in bsa controller", e)
